...
class WatermarkedCanvas(canvas.Canvas):
    """
    A custom canvas class that automatically draws a watermark on every page.
    """
    def __init__(self, *args, **kwargs):
        self._watermark_text = "ealu.pl"
        super().__init__(*args, **kwargs)

    def showPage(self):
        """Draws the watermark before finalizing the page."""
        self.draw_watermark()
        super().showPage()

    def save(self):
        """Draws the watermark on the last page before saving the document."""
        self.draw_watermark()
        super().save()

    def draw_watermark(self):
        """Draws a transparent watermark in the background."""
        self.saveState()
        ...
        self.setFillColor(colors.grey, alpha=0.02)
        
        for x in range(-int(width * 1.5), int(width * 1.5), 350):
            for y in range(-int(height * 1.5), int(height * 1.5), 200):
                self.drawString(x, y, self._watermark_text)
